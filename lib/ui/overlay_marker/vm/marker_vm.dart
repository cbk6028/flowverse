...
import 'package:flov/domain/models/action/action.dart';
import 'package:flov/domain/models/annotation/strikethrough.dart';
import 'package:flov/domain/models/annotation/text_highlighter.dart';
...
// import 'package:flov/domain/models/stroke.dart';
...
class MarkerViewModel extends ChangeNotifier {
  ...
  final Underline _underline = Underline();
  final Strikethrough _strikethrough = Strikethrough();
  final TextHighlighter _highlight = TextHighlighter();
  // 撤销和重做栈
    ...
    switch (currentMarkerType) {
      case MarkerType.highlight:
        for (var range in ranges) {
          Paint paint = Paint()
            ..color = _highlight.color.withValues(alpha: 0.7)
  ...
  // 保存归档到文件
  Future<void> saveArchive(Book book) async {
    _archiveRepository.saveArchive(book, archive);
  }

  Future<void> loadArchive(Book book) async {
    archive = await _archiveRepository.loadArchive(book);
    notifyListeners();
  }
